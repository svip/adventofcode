...
fn run_game(map: HashMap<(i32, i32), bool>, width: usize, height: usize, mut units: HashMap<i32, Unit>, e_power: i32) -> bool {
	let mut round = 0;
	'main: loop {
		...
		while tmp_units.len() > 0 {
			...
			if any_neighbour {
				...
				if hp < 201 {
					...
					u.hp = u.hp - if tmp_unit.is_goblin { 3 } else { e_power };
					if u.hp <= 0 {
						if !u.is_goblin { 
							println!("Noo, an elf died!");
							return false;
						}
						let mut i = 0;
		...
		println!("After round {}: {} units left ({} goblins, {} elves ({}))", round, units.len(), units.iter().filter(|u| u.1.is_goblin).count(), units.iter().filter(|u| !u.1.is_goblin).count(), e_power);
		round += 1;
	}
	units = units.iter().filter(|u| u.1.hp > 0).map(|u| (*u.0, *u.1)).collect();
	...
	println!("Round {}: {} ({}*{}) (power: {})", round, sum * round, sum, round, e_power);
	return true;
}
...
fn main() {
	...
	while !run_game(map.clone(), width, height, units.clone(), attack_power) {
		attack_power += 1;
	}
}

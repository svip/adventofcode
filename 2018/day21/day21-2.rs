...
fn main() {
	...
	// It's basically the same as part 1, except this time we need to keep
	// running, until the number at work_register_at appears again.
	let mut values_found: Vec<u64> = vec![];
	let mut work_register_value = 0;
	...
	loop {
		...
		if work_register_value != registers[work_register_at] && registers[ipr] == eqipr {
			work_register_value = registers[work_register_at];
			if values_found.contains(&work_register_value) {
				// We've begun a loop, now break
				break;
			}
			values_found.push(work_register_value);
		}
		registers[ipr] += 1;
	}
	// Find the last value.
	println!("{}", values_found.pop().unwrap());
}
